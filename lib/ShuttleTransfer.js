...
const {shuttlesMap} = require('./GraphReader');
...
const getStationsTo = (destination, costTable) => {
    ...
    const cost = costTable[destination].cost;
...
const getAllTransfers = stationsOnTheWay => {
    ...
    for(let i=0; i<stationsOnTheWay.length-1; i++){
      ...
      transfers.push({board: from, land: to, shuttles: intersection});
    }
    return transfers;
}
...
module.exports = (destination, costTable) => {
  ...
  return {transfers: getAllTransfers(stationsOnTheWay), totalWeight: costTable[destination].cost, totalPrice: 0};
}
